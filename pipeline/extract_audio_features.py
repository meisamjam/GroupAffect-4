...
For each session × task × participant the script:
  ...
  2. Applies **frame-level energy-ratio bleed rejection**: a 25 ms frame is
     attributed to a mic only when that mic's RMS energy exceeds all other
     mics by at least 1.5.  Frames where no
     mic dominates are counted as overlap/cross-talk.
...
Usage:
    ...
    # Increase bleed-rejection threshold (stricter):
    python tools/features/extract_audio_features.py \\
...
def _build_speech_mask(
    ...
    A frame is accepted when:
      ...
      - target_mic RMS / max(other mics RMS) >= energy_ratio
...
def _discover_sessions(audio_root: Path, groups: list[str] | None) -> list[Path]:
    """Return session directories that contain an audio/ folder."""
    sessions: list[Path] = []
    for d in sorted(audio_root.iterdir()):
        if d.is_dir() and (d / "audio").is_dir():
            sessions.append(d)
    for audio_dir in sorted(audio_root.glob("sub-*/ses-*/audio")):
        if audio_dir.is_dir():
            sessions.append(audio_dir.parent)

    unique = sorted({session.resolve(): session for session in sessions}.values())
    if groups:
        group_lc = [group.lower() for group in groups]
        unique = [
            session
            for session in unique
            if any(group in session.name.lower() for group in group_lc)
        ]
    return unique
...
def _discover_task_mics(
    ...
    for wav in sorted(audio_dir.glob("*.wav")):
        name = wav.name.lower()
        if re.search(r"_aud_hp", name) or "peaks" in wav.parts or "dpa" not in name:
            continue
...
def process_session(
    ...
    for task_id in sorted(task_mics.keys()):
        ...
        for mic, participant_id in MIC_TO_PARTICIPANT.items():
            ...
            # Energy-ratio bleed rejection
            mask = _build_speech_mask(mic_rms, mic, energy_ratio, min_speech_frames)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        ...
        help=(
            "Energy-ratio threshold for bleed rejection (default: 1.5). "
            "A 25 ms frame is attributed to a mic only when its RMS energy "
            "exceeds all other mics by this factor."
        ),
    )
    ap.add_argument(
