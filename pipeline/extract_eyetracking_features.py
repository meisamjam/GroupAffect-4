...
FLAG_DESCRIPTIONS: dict[str, str] = {
    ...
    "blink_detection_unreliable": (
        "Gaze validity is too low (< 70%) to distinguish blinks from tracking failures; "
        "blink_count_est, blink_rate_per_min, and blink_duration_mean_s are set to NaN. "
        "When gaze_valid is stuck at 1.0 (>= 99%), pupil-NaN transitions are used as a "
        "fallback instead of raising this flag."
    ),
    "blink_rate_unusual": (
        "Estimated blink rate (when detection is reliable) is outside the expected range "
        "(2–45 blinks/min); possible systematic gaze-validity artefact."
    ),
    "gaze_out_of_bounds": (
    ...
    "pupil_velocity_spike": (
        "Mean pupil dilation velocity (computed only between consecutive valid samples) "
        "exceeds 10.0 mm/s, suggesting systematic artefactual jumps."
    ),
}
...
FEATURE_DEFINITIONS = [
    ...
    # QC
    ("blink_detectable", "Whether blink detection was possible. True when gaze_valid_frac >= 0.70 (uses gaze transitions) or >= 0.99 (uses pupil-NaN fallback). False when gaze_valid_frac < 0.70."),
...
def _blink_gaps_from_signal(
    invalid_mask: np.ndarray,
    t: np.ndarray,
    min_blink_s: float,
    max_blink_s: float,
) -> list[float]:
    """Return durations of runs in invalid_mask that fall within the blink window."""
    diffs = np.diff(np.concatenate([[0], invalid_mask.astype(int), [0]]))
...
def _estimate_blinks(
    ...
    t: np.ndarray,
    pupil_left: np.ndarray | None = None,
    pupil_right: np.ndarray | None = None,
    min_blink_s: float = 0.05,
    max_blink_s: float = 0.40,
    use_pupil_fallback: bool = False,
) -> tuple[int, float, float]:
    """Detect blink-like gaps from gaze_valid transitions, or pupil NaN gaps as fallback.

    When use_pupil_fallback=True (gaze_valid stuck at 1.0), pupil NaN runs are
    used instead.  Returns (count, mean_duration_s, best_eye_coverage).

    best_eye_coverage is -1.0 when no usable eye was found (fallback failed),
    or the NaN-based coverage (0–1) of the eye selected for blink detection.
    In non-fallback mode it is always 1.0 (not used by the caller).
    """
    if t.size < 3:
        return 0, float("nan"), 1.0

    if use_pupil_fallback:
        # Use the eye with best coverage (fewest NaN) to avoid counting tracking
        # dropouts as blinks. Require > 70% coverage to even consider an eye.
        best: list[float] = []
        best_coverage = -1.0
        for sig in [pupil_left, pupil_right]:
            if sig is None or sig.size < 3:
                continue
            n = min(len(sig), len(t))
            coverage = float(np.mean(np.isfinite(sig[:n])))
            if coverage < 0.70:
                continue
            if coverage > best_coverage:
                gaps = _blink_gaps_from_signal(~np.isfinite(sig[:n]), t[:n], min_blink_s, max_blink_s)
                best = gaps
                best_coverage = coverage
        durations = best
        count = len(durations)
        mean_dur = float(np.mean(durations)) if durations else float("nan")
        return count, mean_dur, best_coverage
    else:
        if gaze_valid.size < 3:
            return 0, float("nan"), 1.0
        invalid = ~(gaze_valid > 0.5)
        durations = _blink_gaps_from_signal(invalid, t, min_blink_s, max_blink_s)
    count = len(durations)
    mean_dur = float(np.mean(durations)) if durations else float("nan")
    return count, mean_dur, 1.0
...
def compute_et_features(
    ...
    blink_rate_min_per_min: float = 2.0,
    blink_rate_max_per_min: float = 45.0,
    blink_valid_frac_min: float = 0.70,
    blink_valid_frac_max: float = 0.99,
    gaze_oob_threshold: float = 0.10,
    pupil_velocity_spike_threshold: float = 10.0,
) -> dict[str, object]:
    ...
    # Pupil velocity — computed only between consecutive valid (non-blink) samples
    # to avoid inflating the mean with near-blink transitions.
    if len(pupil) > 1 and len(gaze_valid) >= len(pupil):
        both_valid = (gaze_valid[: len(pupil) - 1] > 0.5) & (gaze_valid[1 : len(pupil)] > 0.5)
        both_finite = np.isfinite(pupil[:-1]) & np.isfinite(pupil[1:])
        keep = both_valid & both_finite
        if keep.any():
            dpupil = np.abs(np.diff(pupil)[keep])
            dt_safe = np.where(np.diff(t)[keep] > 0, np.diff(t)[keep], np.nan)
            pv = dpupil / dt_safe
    ...
    # ── Blink detection ───────────────────────────────────────────────────
    # Three cases:
    #   normal   (0.70 <= valid_frac < 0.99): use gaze_valid transitions.
    #   fallback (valid_frac >= 0.99): gaze_valid is stuck at 1.0; use pupil
    #       NaN transitions instead (blinks still cause momentary NaN in pupil).
    #   unreliable (valid_frac < 0.70): too many tracking drops to distinguish
    #       blinks — skip and flag blink_detection_unreliable.
    if gaze_valid_frac >= blink_valid_frac_max:
        # gaze_valid stuck at 1.0 — fall back to pupil-NaN transitions.
        blink_count, blink_duration_mean_s, best_eye_cov = _estimate_blinks(
            gaze_valid, t, pupil_left=left, pupil_right=right, use_pupil_fallback=True
        )
        if best_eye_cov < 0:
            # No eye had sufficient coverage — fallback impossible.
            blink_detectable = False
            blink_count = 0
            blink_duration_mean_s = float("nan")
        elif best_eye_cov > 0.98:
            # Eye is near-perfect (device tracks through blinks without marking NaN);
            # pupil-NaN method cannot reliably detect blinks for this participant.
            blink_detectable = False
            blink_count = 0
            blink_duration_mean_s = float("nan")
        else:
            blink_detectable = True
    elif gaze_valid_frac >= blink_valid_frac_min:
        blink_detectable = True
        blink_count, blink_duration_mean_s, _ = _estimate_blinks(gaze_valid, t)
    else:
        blink_detectable = False
        blink_count = 0
        blink_duration_mean_s = float("nan")
    ...
    blink_rate_per_min = (
        ...
        if blink_detectable and np.isfinite(duration_s) and duration_s > 0
        else float("nan")
    )
    ...
    if not blink_detectable:
        flags.append("blink_detection_unreliable")
    elif np.isfinite(blink_rate_per_min) and not (blink_rate_min_per_min <= blink_rate_per_min <= blink_rate_max_per_min):
    ...
    return {
        ...
        "blink_detectable": blink_detectable,
        "qc_flag": _qc_flag_text(flags),
        "qc_notes": _qc_notes_text(flags),
    }
